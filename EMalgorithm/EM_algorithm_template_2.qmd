...
em_algorithm <- function(d, val0) {
        ...
        E <- val0[1] * dnorm(d, val0[2], sqrt(val0[4])) / 
            (val0[1] * dnorm(d, Sk[2], sqrt(val0[4])) + (1 - val0[1]) * 
            dnorm(d, val0[3], sqrt(val0[5])))
    
        # Maximization step 
        valk <- numeric(5)
        valk[1] <- mean(Expect)
        valk[2] <- sum(Expect * d) / sum(Expect) 
        valk[3] <- sum((1-Expect) * d) / (sum(1-Expect))
        valk[4] <- sum(Expect * (d - a2) ** 2) / sum(Expect)
        valk[5] <- sum((1-Expect) * (d - a3) ** 2) / sum(1 - Expect)
        return(valk)
}
