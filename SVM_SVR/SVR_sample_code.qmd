...
![see this video:](https://github.com/stats9/math-and-python/blob/master/SVM_SVR/animation.mp4)
...
fig, ax = plt.figure(2, 2, figsize = (9, 9))
j = -1
i = -1
for k in Tol: 
    Model_temp = SVR(kernel = 'linear', tol = k)
    ...
    for i in range(2): 
        for 
    ax[j].scatter(x = xx, y = yy, color = 'red', s = 50)
    snapp = plt.plot(xx, pred)
    plt.legend(snapp, [f'Tolerance:{k}'])
    camera.snap() 
# animation = camera.animate()
